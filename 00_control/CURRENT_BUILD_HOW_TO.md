...
9. If you open a non-validated project that has grounded runtime page proofs or request-backed traces but still has no grounded launch URL, **Open Runtime** can now keep the runtime workbench active on that evidence surface; embedded launch and Debug Host should still stay blocked for that project until it has a grounded runtime entry.
