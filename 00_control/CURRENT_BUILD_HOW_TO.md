...
17. Donor scan now also writes `evidence/local_only/harvest/capture-blocker-families.json`, which groups those blocker-class targets into reusable families such as `coin`, `big_win`, or `bird`. Use that file or the donor panel summary when you want to review blocked source families instead of reading one long flat URL list.
