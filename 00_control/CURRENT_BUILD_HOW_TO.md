...
5. The donor panel now surfaces the donor scan summary too, so you can see scan state, runtime candidate count, atlas metadata count, bundle-map status, bundle image-variant status/counts, grounded bundle-image URL rule status/counts, mirror status, top next capture targets, grounded alternate hint counts, last guided capture result, blocker summary, and the next operator action without opening raw JSON first.
