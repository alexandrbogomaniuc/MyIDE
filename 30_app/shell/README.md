...
- The shell now has a bounded dedicated Runtime Debug Host path for `project_001` that loads the same local mirror in its own BrowserWindow and is the stronger runtime-trace path in the current validated slice.
...
- The dedicated debug-host smoke now proves a request-backed local static image candidate plus a bounded project-local override hit after reload, so this is the path to use when live runtime asset proof matters.
