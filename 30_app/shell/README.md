...
- Keep the deep donor-scan decision explicit too: the current repo already has a strong bounded partial local runtime package, atlas/frame import is still blocked by missing local source files, and the next best source-discovery lane is the bundle-discovered runtime metadata family rather than more editor-side grouping work.
...
- The deep donor scan now makes that blocker more precise: `bundle.js` references concrete runtime metadata paths such as `img/spines/*.json`, `img/coins/coin.json`, `img/ui/logo.png`, and translations, but those files are not present locally yet.
