...
- The first embedded guest-preload runtime introspection bridge is now active for `project_001`, and the current strongest honest result is explicit: the embedded path still reports `frameCount=0`, `accessibleFrameCount=0`, `canvasCount=0`, resource windows stay at `top`, and no stable display-object or request-backed static image is exposed yet.
