...
- Explain current scope inside the shell with a compact Start Here panel, quickstart, current-scope guidance, a read-only donor evidence browser with donor/importer drill-down, small copy helpers for donor IDs / paths / evidence refs, and selected-object evidence linkage so testers understand the build edits reconstructed internal scene files rather than raw donor assets directly.
