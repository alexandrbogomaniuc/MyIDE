...
- Keep the local runtime boundary explicit and documented in `00_control/RUNTIME_PACKAGE_NOTES.md`: partial local mirror exists, full local donor package still does not, and the current mirrored static override candidate still does not prove a reload-time hit yet.
