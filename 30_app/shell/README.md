...
- Keep the local runtime boundary explicit and documented in `00_control/RUNTIME_PACKAGE_NOTES.md`: partial local mirror exists, full local donor package still does not, the strongest current proof shows zero observed upstream static-image requests plus one remaining unresolved upstream bootstrap `bundle.js` request, and the current mirrored static override candidate still does not prove a reload-time hit yet.
