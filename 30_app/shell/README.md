...
- Explain current scope inside the shell with a compact Start Here panel, quickstart, current-scope guidance, and a read-only donor evidence summary so testers understand the build edits reconstructed internal scene files rather than raw donor assets directly.
