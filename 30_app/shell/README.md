...
- The shell now treats the embedded runtime webview as a no-go for stronger runtime-trace proof in this environment and adds a bounded dedicated Runtime Debug Host path for `project_001` that loads the same local mirror in its own BrowserWindow.
...
- The new decisive blocker is environment-level: both the embedded smoke and the dedicated debug-host smoke are currently aborting in macOS AppKit before MyIDE emits its own smoke markers, so the stronger trace path exists in code but still needs a healthy Electron GUI run to prove live object/asset truth.
