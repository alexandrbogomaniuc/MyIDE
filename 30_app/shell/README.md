...
- Keep the local runtime boundary explicit and documented in `00_control/RUNTIME_PACKAGE_NOTES.md`: partial local mirror exists, full local donor package still does not, the strongest current proof shows zero observed upstream static-image requests with no unresolved upstream bootstrap/static dependency in the bounded cycle, and the current strongest override candidate is still only mirror-manifest-backed.
