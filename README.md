...
- The current strongest local-runtime proof is narrower and better grounded: the local mirror now serves the observed launch path by default on this machine, the bounded slice currently shows zero observed upstream static-image requests, and the latest live proof leaves no unresolved upstream bootstrap/static dependency in the current bounded cycle.
