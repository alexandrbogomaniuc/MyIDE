...
- The shell now has one usable runtime-debug path for `project_001`: a bounded dedicated Runtime Debug Host that loads the same local mirror in a separate BrowserWindow and can prove a request-backed static image override hit.
...
- The decisive product conclusion is now clearer: embedded Runtime Mode remains weaker for live asset ownership, but the dedicated Runtime Debug Host path is the stronger local runtime workflow for asset tracing and override proof in the current validated slice.
