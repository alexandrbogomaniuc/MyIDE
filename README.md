...
- The shell now has one official runtime workbench path for `project_001`: the bounded dedicated Runtime Debug Host loads the same local mirror in a separate BrowserWindow, anchors the runtime asset/request workbench inside the shell, and is the path to use first when runtime asset tracing or override proof matters. Embedded Runtime remains secondary/weaker for now.
...
- The decisive product conclusion is now clearer: embedded Runtime Mode remains weaker and secondary for this slice, while the dedicated Runtime Debug Host is the official day-to-day runtime workflow for asset tracing, source jumps, compose bridging, override proof, and the practical runtime workbench loop in the current validated slice.
...
- Daily entry point:
  - `./Start-MyIDE.command` (Finder, repo root)
  - `./run/Start-MyIDE.command` (Finder, inside `run/`)
...
If you are unsure which one to use, double-click `./Start-MyIDE.command`.
