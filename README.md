...
- The shell now treats the embedded runtime path as a no-go for stronger asset-level proof in this environment and adds a bounded dedicated Runtime Debug Host path for `project_001` that loads the same local mirror in a separate BrowserWindow.
...
- The current decisive blocker is environment-level: both the embedded runtime smoke and the new debug-host smoke are now aborting in macOS AppKit before MyIDE emits its own smoke markers, so the stronger runtime-trace proof still needs a healthy Electron GUI session before it can be completed.
