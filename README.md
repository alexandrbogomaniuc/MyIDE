...
- Donor scan now also has a guided capture runner: `npm run donor-scan:capture-next -- --donor-id donor_XXX --limit 5` attempts the top ranked missing donor/runtime files, writes `next-capture-run.json`, refreshes donor scan, and tells the operator whether the blocker queue actually shrank.
