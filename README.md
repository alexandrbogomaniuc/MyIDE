...
- The current bounded runtime proof now covers a local runtime mirror launch URL, a grounded local mirror source-path trace for one static Mystery Garden runtime candidate, override creation/cleanup for that candidate, and explicit blocker reporting when the mirrored candidate does not record a reload-time hit yet.
