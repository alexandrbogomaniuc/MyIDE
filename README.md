...
- The new deep donor scan now answers the next product-direction question plainly for `project_001`: a strong bounded partial local runtime package already exists, a full standalone local donor runtime package still does not, atlas/frame import is still blocked on missing local source files, and the next best source-discovery path is the bundle-discovered runtime metadata family (`img/spines/*.json`, `img/coins/coin.json`, `img/ui/logo.png`, translation roots).
...
- Atlas/frame donor import is still blocked for `project_001` on this machine because no local atlas text, sprite-sheet JSON, plist, or other grounded frame metadata source has been captured yet. The deep donor scan now proves the likely runtime metadata paths exist in `bundle.js`, but the referenced local files are still missing, so the donor palette remains limited to loose donor images for now.
