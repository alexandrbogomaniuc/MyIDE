...
- The shell now prefers a stronger main-world `executeJavaScript` runtime introspection bridge before falling back to the older guest-preload bridge inside the embedded runtime webview for `project_001`.
...
- This run landed the main-world bridge code path, but the Electron smoke environment started aborting in macOS AppKit before MyIDE emitted its main-process smoke marker, so the new bridge could not be end-to-end verified in this environment.
