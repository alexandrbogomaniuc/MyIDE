...
- The first live-runtime introspection bridge is now active inside the embedded runtime webview, and the current strongest result is explicit: `frameCount=0`, `accessibleFrameCount=0`, `canvasCount=0`, the only observed resource window is `top`, and the embedded path still does not expose a request-backed static image or display-object handle for override proof.
