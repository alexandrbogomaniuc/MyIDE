...
- The current shell is a bounded internal scene editor for `40_projects/project_001/internal`; donor evidence under `10_donors/` now appears in a read-only donor evidence browser with drill-down sections, copyable refs/paths, and selected-object linkage summary, but it remains reference material rather than directly editable assets in this build.
