...
The current shell also surfaces a bounded runtime request map for the active launch/reload cycle so testers can see which runtime URLs actually resolved to local mirror files or active override files. The strongest current proof now shows zero observed upstream static-image requests in the bounded slice, no unresolved upstream bootstrap/static dependency in the current cycle, and the same honest override blocker as before: the strongest current candidate is still mirror-manifest-backed rather than request-backed.
