...
- The harvested mirror can now feed the generic donor-scan subsystem too, so an existing mirrored donor/runtime package can still produce entry-point, bundle-map, atlas-manifest, runtime-candidate, and blocker-summary outputs even when it was not originally created through donor URL intake.
- The mirror capture now also chases bundle-discovered runtime metadata candidates when they are grounded, including deeper `img/spines/*.json`, `img/coins/*.json`, sibling `.atlas`, atlas page images, and translation roots when those URLs resolve.
