...
async function main(): Promise<void> {
  ...
  assert.ok(typeof scanSummary.sectionSkinTextureRenderBundleCount === "number", "scan summary should record section skin texture render bundle counts");
  assert.ok(Array.isArray(scanSummary.topSectionSkinTextureRenderBundleKeys), "scan summary should record section skin texture render bundle keys");
  assert.ok(typeof scanSummary.sectionSkinTextureCanvasBundleCount === "number", "scan summary should record section skin texture canvas bundle counts");
  ...
  assert.ok(typeof scanSummary.sectionSkinTextureFitDecisionBundleCount === "number", "scan summary should record section skin texture fit-decision bundle counts");
  assert.ok(Array.isArray(scanSummary.topSectionSkinTextureFitDecisionBundleKeys), "scan summary should record section skin texture fit-decision bundle keys");
  assert.ok(typeof scanSummary.sectionSkinTextureFitApprovalBundleCount === "number", "scan summary should record section skin texture fit-approval bundle counts");
  assert.ok(Array.isArray(scanSummary.topSectionSkinTextureFitApprovalBundleKeys), "scan summary should record section skin texture fit-approval bundle keys");
  assert.ok(typeof scanSummary.sectionSkinTextureFitApplyBundleCount === "number", "scan summary should record section skin texture fit-apply bundle counts");
  assert.ok(Array.isArray(scanSummary.topSectionSkinTextureFitApplyBundleKeys), "scan summary should record section skin texture fit-apply bundle keys");
  ...
  const hasSectionSkinTextureRenderBundleProfiles = await fileExists(paths.sectionSkinTextureRenderBundleProfilesPath);
  let sectionSkinTextureRenderBundleProfilesCount: number | null = null;
  let topSectionSkinTextureRenderBundleKeys: string[] = [];
  if (hasSectionSkinTextureRenderBundleProfiles) {
    const sectionSkinTextureRenderBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureRenderBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureRenderBundleProfiles.sectionCount === "number", "section skin texture render bundle profiles should record section counts");
    sectionSkinTextureRenderBundleProfilesCount = sectionSkinTextureRenderBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureRenderBundleKeys = Array.isArray(sectionSkinTextureRenderBundleProfiles.sections)
      ? sectionSkinTextureRenderBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  }
  const hasSectionSkinTextureCanvasBundleProfiles = await fileExists(paths.sectionSkinTextureCanvasBundleProfilesPath);
  let sectionSkinTextureCanvasBundleProfilesCount: number | null = null;
  let topSectionSkinTextureCanvasBundleKeys: string[] = [];
  if (hasSectionSkinTextureCanvasBundleProfiles) {
    const sectionSkinTextureCanvasBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureCanvasBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureCanvasBundleProfiles.sectionCount === "number", "section skin texture canvas bundle profiles should record section counts");
    sectionSkinTextureCanvasBundleProfilesCount = sectionSkinTextureCanvasBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureCanvasBundleKeys = Array.isArray(sectionSkinTextureCanvasBundleProfiles.sections)
      ? sectionSkinTextureCanvasBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  }
  const hasSectionSkinTextureSourceFitBundleProfiles = await fileExists(paths.sectionSkinTextureSourceFitBundleProfilesPath);
  let sectionSkinTextureSourceFitBundleProfilesCount: number | null = null;
  let topSectionSkinTextureSourceFitBundleKeys: string[] = [];
  if (hasSectionSkinTextureSourceFitBundleProfiles) {
    const sectionSkinTextureSourceFitBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureSourceFitBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureSourceFitBundleProfiles.sectionCount === "number", "section skin texture source-fit bundle profiles should record section counts");
    sectionSkinTextureSourceFitBundleProfilesCount = sectionSkinTextureSourceFitBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureSourceFitBundleKeys = Array.isArray(sectionSkinTextureSourceFitBundleProfiles.sections)
      ? sectionSkinTextureSourceFitBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  ...
  const hasSectionSkinTextureFitDecisionBundleProfiles = await fileExists(paths.sectionSkinTextureFitDecisionBundleProfilesPath);
  let sectionSkinTextureFitDecisionBundleProfilesCount: number | null = null;
  let topSectionSkinTextureFitDecisionBundleKeys: string[] = [];
  if (hasSectionSkinTextureFitDecisionBundleProfiles) {
    const sectionSkinTextureFitDecisionBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureFitDecisionBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureFitDecisionBundleProfiles.sectionCount === "number", "section skin texture fit-decision bundle profiles should record section counts");
    sectionSkinTextureFitDecisionBundleProfilesCount = sectionSkinTextureFitDecisionBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureFitDecisionBundleKeys = Array.isArray(sectionSkinTextureFitDecisionBundleProfiles.sections)
      ? sectionSkinTextureFitDecisionBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  }
  const hasSectionSkinTextureFitApprovalBundleProfiles = await fileExists(paths.sectionSkinTextureFitApprovalBundleProfilesPath);
  let sectionSkinTextureFitApprovalBundleProfilesCount: number | null = null;
  let topSectionSkinTextureFitApprovalBundleKeys: string[] = [];
  if (hasSectionSkinTextureFitApprovalBundleProfiles) {
    const sectionSkinTextureFitApprovalBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureFitApprovalBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureFitApprovalBundleProfiles.sectionCount === "number", "section skin texture fit-approval bundle profiles should record section counts");
    sectionSkinTextureFitApprovalBundleProfilesCount = sectionSkinTextureFitApprovalBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureFitApprovalBundleKeys = Array.isArray(sectionSkinTextureFitApprovalBundleProfiles.sections)
      ? sectionSkinTextureFitApprovalBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  }
  const hasSectionSkinTextureFitApplyBundleProfiles = await fileExists(paths.sectionSkinTextureFitApplyBundleProfilesPath);
  let sectionSkinTextureFitApplyBundleProfilesCount: number | null = null;
  let topSectionSkinTextureFitApplyBundleKeys: string[] = [];
  if (hasSectionSkinTextureFitApplyBundleProfiles) {
    const sectionSkinTextureFitApplyBundleProfiles = await readJsonFile<{
      sectionCount?: number;
      sections?: Array<{ sectionKey?: string }>;
    }>(paths.sectionSkinTextureFitApplyBundleProfilesPath);
    assert.ok(typeof sectionSkinTextureFitApplyBundleProfiles.sectionCount === "number", "section skin texture fit-apply bundle profiles should record section counts");
    sectionSkinTextureFitApplyBundleProfilesCount = sectionSkinTextureFitApplyBundleProfiles.sectionCount ?? 0;
    topSectionSkinTextureFitApplyBundleKeys = Array.isArray(sectionSkinTextureFitApplyBundleProfiles.sections)
      ? sectionSkinTextureFitApplyBundleProfiles.sections
          .map((section) => typeof section?.sectionKey === "string" ? section.sectionKey : "")
          .filter((value) => value.length > 0)
          .slice(0, 6)
      : [];
  ...
  if (sectionSkinTextureRenderBundleProfilesCount !== null) {
    console.log(`Section skin texture render bundle profiles: ${sectionSkinTextureRenderBundleProfilesCount} (${topSectionSkinTextureRenderBundleKeys.join(", ")})`);
  }
  if (sectionSkinTextureCanvasBundleProfilesCount !== null) {
    console.log(`Section skin texture canvas bundle profiles: ${sectionSkinTextureCanvasBundleProfilesCount} (${topSectionSkinTextureCanvasBundleKeys.join(", ")})`);
  }
  ...
  if (sectionSkinTextureFitDecisionBundleProfilesCount !== null) {
    console.log(`Section skin texture fit-decision bundle profiles: ${sectionSkinTextureFitDecisionBundleProfilesCount} (${topSectionSkinTextureFitDecisionBundleKeys.join(", ")})`);
  }
  if (sectionSkinTextureFitApprovalBundleProfilesCount !== null) {
    console.log(`Section skin texture fit-approval bundle profiles: ${sectionSkinTextureFitApprovalBundleProfilesCount} (${topSectionSkinTextureFitApprovalBundleKeys.join(", ")})`);
  }
  if (sectionSkinTextureFitApplyBundleProfilesCount !== null) {
    console.log(`Section skin texture fit-apply bundle profiles: ${sectionSkinTextureFitApplyBundleProfilesCount} (${topSectionSkinTextureFitApplyBundleKeys.join(", ")})`);
  }
