...
The deep donor scan now proves the likely runtime metadata paths exist in `bundle.js`, but the referenced local files are still missing locally, so atlas/frame import remains blocked by missing source material.
