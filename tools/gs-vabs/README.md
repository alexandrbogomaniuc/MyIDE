...
- `npm run vabs:parse:project_001`
  ...
  - Print the target folder decision, resolved fixture tier, parsed `ROUND_ID`, and payload keys.
- `npm run vabs:replay:project_001`
  ...
  - Write deterministic replay-summary artifacts to `/tmp/myide-vabs-project_001-replay/<fixture>/`.
- `npm run vabs:verify:project_001`
  ...
  - Check captured-vs-derived tracking is explicit through `captured-row-notes.md` when no full captured row exists yet.
...
- Local-first.
- Deterministic.
- Scaffold, captured-vs-derived fixture tracking, local replay-summary harness, and verification only.
