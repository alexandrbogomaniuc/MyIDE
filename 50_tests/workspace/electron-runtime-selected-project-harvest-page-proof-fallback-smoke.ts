...
function getArtifactPath(): string {
  return process.env.MYIDE_LIVE_RUNTIME_SELECTED_PROJECT_HARVEST_PAGE_PROOF_FALLBACK_ARTIFACT_PATH
    || "/tmp/myide-electron-runtime-selected-project-harvest-page-proof-fallback.json";
}
...
async function main(): Promise<void> {
  ...
  const sourceBundlePath = path.join(reportsRoot, "selected-project-runtime-harvest-page-proof-fallback.smoke.json");
  const sourceRuntimeBundlePath = path.join(reportsRoot, "selected-project-runtime-harvest-page-proof-fallback", "big-win.bundle.js");
  const sourceRuntimeImagePath = path.join(reportsRoot, "selected-project-runtime-harvest-page-proof-fallback", "big-win-ribbon.png");
  ...
  const taskId = "task.runtime.harvest.page-proof-fallback.project_002.smoke";
  ...
  try {
    ...
    const runtimeProofRepoRelativePath = `40_projects/${projectId}/runtime/page-runtime-proofs.latest.json`;

    await fs.writeFile(sourceRuntimeBundlePath, "console.log('project_002_runtime_harvest_page_proof_fallback_source_placeholder');\n", "utf8");
    await fs.writeFile(sourceRuntimeImagePath, png1x1);
    await fs.writeFile(mirrorBundlePath, "console.log('project_002_runtime_harvest_page_proof_fallback_bundle_placeholder');\n", "utf8");
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        generatedAt,
        nextTextureFitApplyStep: "Harvest request-backed runtime traces from persisted grounded page proofs even though no mirror manifest or request log is available.",
        pages: [
          {
            pageName,
            pageState: "ready",
            selectedMode: "page-source",
            selectedReason: "Smoke-seeded selected-project runtime harvest from persisted page-proof bundle evidence without a mirror manifest or request log.",
            ...
            nextFitApplyStep: "Harvest request-backed runtime evidence from the persisted page proof and keep the selected-project runtime workbench on that bundle trace while launch stays blocked."
          },
          {
            pageName: "big_win_image",
            pageState: "ready",
            selectedMode: "page-source",
            selectedReason: "Smoke-seeded task-kit image bridge for page-proof fallback static-image override proof.",
            ...
            nextFitApplyStep: "Use the harvested page-proof-backed static image as the bounded override candidate while the task runtime entry stays on the bundle."
          }
        ],
        localSources: [
          {
            ...
            relation: "page-proof-fallback-static-override"
          }
        ]
      }, null, 2)}\n`,
      "utf8"
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        ...
        readyTaskCount: 1,
        blockedTaskCount: 0,
        nextOperatorAction: "Harvest request-backed runtime traces from persisted page-proof evidence, open Runtime from the modification board on that bundle entry, then create a bounded override from the harvested static image.",
        tasks: [
          {
            taskId,
            queueId: "smoke.queue.project_002.runtime_harvest_page_proof_fallback",
            scenarioId: "smoke.scenario.project_002.runtime_harvest_page_proof_fallback",
            displayName: "Project 002 Runtime Harvest Page-proof Fallback Smoke",
            ...
            rationale: "Smoke-seeded modification task proving selected-project runtime harvest can refresh grounded page-proof runtime evidence without a mirror manifest or request log, then keep the task on the bundle trace while bounded static-image override proof still works.",
            nextAction: "Harvest Runtime requests from persisted page-proof evidence, verify the task reopens on the bundle trace, and create/clear the bounded override from the harvested static image.",
            canOpenCompose: true,
            canOpenRuntime: true
          }
        ]
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        ...
        snapshotRepoRelativePath: runtimeProofRepoRelativePath,
        entryCount: 2,
        entries: [
          {
            taskId,
            ...
            runtimeLabel: "Big Win Bundle",
            profileId: null,
            requestSource: "local-mirror-proxy",
            requestBacked: true,
            relativePath: "big-win.bundle.js",
    ...
    assert(mainReadySeen, `Main-process selected-project harvest page-proof fallback marker was not emitted. Exit=${run.exitCode} Signal=${run.signal ?? "none"}\n${output}`);

    const markerPrefix = "MYIDE_LIVE_RUNTIME_SELECTED_PROJECT_HARVEST_RESULT:";
    const payloadLine = lines.find((line) => line.startsWith(markerPrefix));
    assert(payloadLine, `Renderer selected-project harvest page-proof fallback result marker was not emitted. Exit=${run.exitCode} Signal=${run.signal ?? "none"}\n${output}`);
    ...
    assert((payload.harvestedEntryCount ?? 0) >= 2, "Selected-project harvest page-proof fallback smoke should record both the bundle and static-image runtime entries.");
    assert.equal(payload.harvestApiOverrideEntryCount ?? 0, 0, "Selected-project harvest page-proof fallback smoke should not claim override-backed hits before the override exists.");
    assert.equal(payload.harvestedRequestCategory, "html-bootstrap", "Selected-project harvest page-proof fallback smoke should treat the bundle as a runtime dependency.");
    assert.equal(payload.harvestedRequestSource, "local-mirror-proxy", "Selected-project harvest page-proof fallback smoke should refresh the bundle through the proxy path.");
    assert.equal(payload.harvestedStaticAssetSourceUrl, runtimeStaticSourceUrl, "Selected-project harvest page-proof fallback smoke should preserve the static-image source.");
    assert.equal(payload.harvestedStaticAssetRequestCategory, "static-asset", "Selected-project harvest page-proof fallback smoke should preserve the static-image category.");
    assert.equal(payload.harvestedStaticAssetRequestSource, "local-mirror-proxy", "Selected-project harvest page-proof fallback smoke should refresh the static image through the proxy path.");
    assert.equal(payload.taskId, taskId, "Selected-project harvest page-proof fallback smoke should keep the prepared task active.");
    assert.equal(payload.pageName, pageName, "Selected-project harvest page-proof fallback smoke should keep the prepared page name.");
    assert.notEqual(payload.taskRuntimeEntryKind, "page-proof", "Selected-project harvest page-proof fallback smoke should upgrade the task match label beyond the weaker page-proof fallback after harvest.");
    assert.equal(payload.taskRuntimeEntrySourceUrl, runtimeSourceUrl, "Selected-project harvest page-proof fallback smoke used the wrong runtime source.");
    assert.equal(payload.runtimeWorkbenchEntryKind, "resource-map", "Selected-project harvest page-proof fallback smoke should upgrade to a request-backed runtime workbench entry.");
    assert.equal(payload.runtimeModeSelected, true, "Selected-project harvest page-proof fallback smoke should keep Runtime Mode active.");
    assert.equal(payload.taskRuntimeOpenUsesHarvestedRequestWorkbenchEntry, true, "Selected-project harvest page-proof fallback smoke should reopen on the harvested request-backed workbench entry.");
    assert.equal(payload.runtimeOverrideEligible, true, "Selected-project harvest page-proof fallback smoke should expose an override-eligible static image.");
    assert.equal(payload.runtimeOverrideSourceUrl, runtimeStaticSourceUrl, "Selected-project harvest page-proof fallback smoke should promote the static image as the override target.");
    assert.equal(payload.runtimeOverrideRequestSource, "local-mirror-proxy", "Selected-project harvest page-proof fallback smoke should preserve the page-proof-backed static image request source.");
    assert.equal(payload.runtimeOverrideDonorAssetId, donorAssetId, "Selected-project harvest page-proof fallback smoke should bridge the harvested static image back to the task-kit donor asset.");
    assert.equal(payload.createOverrideButtonEnabled, true, "Selected-project harvest page-proof fallback smoke should enable Create Override.");
    assert.equal(payload.runtimeOverrideCreated, true, "Selected-project harvest page-proof fallback smoke did not create the bounded override.");
    assert.equal(payload.runtimeOverrideManifestRepoRelativePath, "40_projects/project_002/overrides/runtime-asset-overrides.json", "Selected-project harvest page-proof fallback smoke wrote the wrong override manifest path.");
    assert.equal(payload.runtimeOverrideRepoRelativePath, path.relative(workspaceRoot, overrideFilePath).replace(/\\/g, "/"), "Selected-project harvest page-proof fallback smoke wrote the wrong override file path.");
    assert.equal(payload.overrideProofHarvestSucceeded, true, "Selected-project harvest page-proof fallback smoke should prove an override-backed hit after re-harvest.");
    ...
    assert.equal(payload.embeddedRuntimeLaunched, false, "Selected-project harvest page-proof fallback smoke should not launch the embedded runtime.");

    console.log("PASS smoke:electron-runtime-selected-project-harvest-page-proof-fallback");
    ...
    await cleanupEmptyDirectory(path.join(reportsRoot, "selected-project-runtime-harvest-page-proof-fallback"));
    ...
    assert.equal(
      restoredStatus,
      baselineStatus,
      `Selected-project harvest page-proof fallback smoke should restore the repo status.\nBefore:\n${baselineStatus || "<clean>"}\nAfter:\n${restoredStatus || "<clean>"}`
    );
  }
}
...
main().catch((error) => {
  ...
  console.error(`FAIL smoke:electron-runtime-selected-project-harvest-page-proof-fallback - ${message}`);
  process.exitCode = 1;
});
