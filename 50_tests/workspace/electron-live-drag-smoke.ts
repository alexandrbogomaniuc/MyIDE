...
interface LiveDragPayload {
  ...
  canvasSelectionSucceeded?: boolean;
  dragStarted?: boolean;
  dragMoved?: boolean;
...
function getArtifactPath(): string {
  return process.env.MYIDE_LIVE_DRAG_ARTIFACT_PATH || "/tmp/myide-electron-live-drag.json";
}

function getScreenshotPath(): string | null {
  const value = process.env.MYIDE_LIVE_DRAG_SCREENSHOT_PATH ?? "";
...
function runElectronLiveDragSmoke(workspaceRoot: string): Promise<SmokeRunResult> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(electronBinary, ["."], {
      ...
      env: {
        ...process.env,
        MYIDE_LIVE_DRAG_SMOKE: "1",
        MYIDE_LIVE_DRAG_TIMEOUT_MS: process.env.MYIDE_LIVE_DRAG_TIMEOUT_MS || "45000",
        ...(screenshotPath ? {
          MYIDE_LIVE_DRAG_SHOW: "1",
          MYIDE_LIVE_DRAG_KEEP_OPEN: "1"
        } : {})
      },
      stdio: ["ignore", "pipe", "pipe"]
    });
    ...
    const markerPrefix = "MYIDE_LIVE_DRAG_RESULT:";
    ...
    const handleChunk = (chunk: unknown) => {
      ...
      while (lineBuffer.includes("\n")) {
        ...
        let payload: LiveDragPayload | null = null;
        try {
          payload = JSON.parse(line.slice(markerPrefix.length)) as LiveDragPayload;
        } catch (error) {
...
async function main(): Promise<void> {
  ...
  const backupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "myide-live-drag-backup-"));
  ...
  try {
    result = await runElectronLiveDragSmoke(workspaceRoot);
    ...
    const mainReadySeen = lines.some((line) => line.includes("MYIDE_LIVE_DRAG_MAIN_READY"));
    assert(
      mainReadySeen,
      `Main-process live-drag marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );
    ...
    assert(
      payloadLine,
      `Renderer live-drag result marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );

    payload = JSON.parse(payloadLine.slice(markerPrefix.length)) as LiveDragPayload;
    assert.equal(payload.status, "pass", `Live drag smoke reported failure: ${payload.error ?? "unknown error"}`);
    ...
    assert.equal(payload.canvasSelectionSucceeded, true, "Renderer did not select the canvas object.");
    assert.equal(payload.dragStarted, true, "Renderer did not start the live canvas drag.");
    assert.equal(payload.dragMoved, true, "Renderer did not move the object through the live canvas drag path.");
    assert.equal(payload.dragCompleted, true, "Renderer did not complete the live canvas drag.");
    assert.equal(payload.saveSucceeded, true, "Renderer did not complete save after the live drag.");
    assert.equal(payload.reloadSucceeded, true, "Renderer did not complete reload after the live drag.");
    assert.equal(payload.internalPersistVerified, true, "Renderer did not confirm the reloaded internal drag position.");
    assert.equal(payload.replaySyncVerified, true, "Renderer did not confirm replay-facing sync after drag.");

    const objectId = payload.objectId ?? "node.bottom-bar";
    ...
    assert.equal(onDiskInternalPosition.x, expectedX, "Editable objects.json did not persist the dragged x coordinate.");
    assert.equal(onDiskInternalPosition.y, expectedY, "Editable objects.json did not persist the dragged y coordinate.");
    assert.equal(onDiskReplayPosition.x, expectedX, "Replay-facing project.json did not persist the dragged x coordinate.");
    assert.equal(onDiskReplayPosition.y, expectedY, "Replay-facing project.json did not persist the dragged y coordinate.");
    assert.equal(
      result.exitCode,
      0,
      `Electron live-drag smoke exited with code ${result.exitCode} (signal ${result.signal ?? "none"}).\n${outputSummary}`
    );
  } catch (error) {
    writeArtifact({
      status: "fail",
  ...
  assert.equal(
    restoredStatus,
    baselineStatus,
    `Repository status diverged after restoring the live-drag smoke.\nBefore:\n${baselineStatus || "<clean>"}\nAfter:\n${restoredStatus || "<clean>"}`
  );
  ...
  console.log("PASS smoke:electron-live-drag");
  console.log(`Dragged object: ${payload?.objectId} (${payload?.originalX}, ${payload?.originalY}) -> (${payload?.draggedX}, ${payload?.draggedY})`);
...
main().catch((error: unknown) => {
  ...
  console.error(`FAIL smoke:electron-live-drag - ${message}`);
  process.exitCode = 1;
});
