...
interface LiveAlignPayload {
  ...
  alignedX?: number | null;
  alignedY?: number | null;
...
function getArtifactPath(): string {
  return process.env.MYIDE_LIVE_ALIGN_ARTIFACT_PATH || "/tmp/myide-electron-live-align.json";
}

function getScreenshotPath(): string | null {
  const value = process.env.MYIDE_LIVE_ALIGN_SCREENSHOT_PATH ?? "";
...
function runElectronLiveAlignSmoke(workspaceRoot: string): Promise<SmokeRunResult> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(electronBinary, ["."], {
      ...
      env: {
        ...process.env,
        MYIDE_LIVE_ALIGN_SMOKE: "1",
        MYIDE_LIVE_ALIGN_TIMEOUT_MS: process.env.MYIDE_LIVE_ALIGN_TIMEOUT_MS || "45000",
        ...(screenshotPath ? {
          MYIDE_LIVE_ALIGN_SHOW: "1",
          MYIDE_LIVE_ALIGN_KEEP_OPEN: "1"
        } : {})
      },
      stdio: ["ignore", "pipe", "pipe"]
    });
    ...
    const markerPrefix = "MYIDE_LIVE_ALIGN_RESULT:";
    ...
    const handleChunk = (chunk: unknown) => {
      ...
      while (lineBuffer.includes("\n")) {
        ...
        let payload: LiveAlignPayload | null = null;
        try {
          payload = JSON.parse(line.slice(markerPrefix.length)) as LiveAlignPayload;
        } catch (error) {
...
async function main(): Promise<void> {
  ...
  try {
    result = await runElectronLiveAlignSmoke(workspaceRoot);
    ...
    const mainReadySeen = lines.some((line) => line.includes("MYIDE_LIVE_ALIGN_MAIN_READY"));
    assert(
      mainReadySeen,
      `Main-process live align marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );
    ...
    assert(
      payloadLine,
      `Renderer live align result marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );

    payload = JSON.parse(payloadLine.slice(markerPrefix.length)) as LiveAlignPayload;
    assert.equal(payload.status, "pass", `Live align smoke reported failure: ${payload.error ?? "unknown error"}`);
    ...
    assert.equal(payload.aligned, true, "Renderer did not apply the viewport alignment action.");
    assert.equal(payload.saveSucceeded, true, "Renderer did not complete save after the live alignment.");
    assert.equal(payload.reloadSucceeded, true, "Renderer did not complete reload after the live alignment.");
    assert.equal(payload.internalPersistVerified, true, "Renderer did not confirm the reloaded internal alignment state.");
    assert.equal(payload.replaySyncVerified, true, "Renderer did not confirm replay-facing sync after the live alignment.");

    const objectId = payload.objectId;
    assert(objectId, "Renderer did not report the created object id.");
    const expectedLayerId = payload.createdLayerId;
    const expectedX = Number(payload.alignedX);
    const expectedY = Number(payload.alignedY);
    const expectedWidth = Number(payload.createdWidth);
    const expectedHeight = Number(payload.createdHeight);
    assert(expectedLayerId, "Renderer did not report the created layer id.");
    assert(Number.isFinite(expectedX) && Number.isFinite(expectedY), "Renderer aligned coordinates must be numeric.");
    assert(Number.isFinite(expectedWidth) && Number.isFinite(expectedHeight), "Renderer width/height must be numeric.");
    ...
    assert.equal(onDiskInternalObject.x, expectedX, "Editable objects.json did not persist the created object's aligned x coordinate.");
    assert.equal(onDiskInternalObject.y, expectedY, "Editable objects.json did not persist the created object's aligned y coordinate.");
    assert.equal(onDiskInternalObject.width, expectedWidth, "Editable objects.json changed width during alignment.");
    assert.equal(onDiskInternalObject.height, expectedHeight, "Editable objects.json changed height during alignment.");
    ...
    assert.equal(onDiskReplayObject.x, expectedX, "Replay-facing project.json did not persist the created object's aligned x coordinate.");
    assert.equal(onDiskReplayObject.y, expectedY, "Replay-facing project.json did not persist the created object's aligned y coordinate.");
    assert.equal(onDiskReplayObject.width, expectedWidth, "Replay-facing project.json changed width during alignment.");
    assert.equal(onDiskReplayObject.height, expectedHeight, "Replay-facing project.json changed height during alignment.");
    assert.equal(
      result.exitCode,
      0,
      `Electron live align smoke exited with code ${result.exitCode} (signal ${result.signal ?? "none"}).\n${outputSummary}`
    );
  } catch (error) {
    writeArtifact({
      status: "fail",
  ...
  assert.equal(
    restoredStatus,
    baselineStatus,
    `Repository status diverged after restoring the live align smoke.\nBefore:\n${baselineStatus || "<clean>"}\nAfter:\n${restoredStatus || "<clean>"}`
  );
  ...
  console.log("PASS smoke:electron-live-align");
  console.log(`Created and aligned object: ${payload?.objectId} (${payload?.createdX}, ${payload?.createdY}) -> (${payload?.alignedX}, ${payload?.alignedY}) via ${payload?.alignmentAction}`);
...
main().catch((error: unknown) => {
  ...
  console.error(`FAIL smoke:electron-live-align - ${message}`);
  process.exitCode = 1;
});
