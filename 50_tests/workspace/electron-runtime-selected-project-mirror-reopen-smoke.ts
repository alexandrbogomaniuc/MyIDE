...
function getArtifactPath(): string {
  return process.env.MYIDE_LIVE_RUNTIME_SELECTED_PROJECT_MIRROR_REOPEN_ARTIFACT_PATH || "/tmp/myide-electron-runtime-selected-project-mirror-reopen.json";
}
...
function runElectronSelectedProjectMirrorReopenSmoke(workspaceRoot: string): Promise<SmokeRunResult> {
  return new Promise((resolve, reject) => {
...
async function main(): Promise<void> {
  ...
  const sourceBundlePath = path.join(reportsRoot, "selected-project-runtime-mirror-reopen.smoke.json");
  const sourceAssetPath = path.join(reportsRoot, "selected-project-runtime-mirror-reopen", "big-win-ribbon.png");
  const mirrorManifestPath = path.join(runtimeRoot, "local-mirror", "manifest.json");
  const mirrorAssetPath = path.join(runtimeRoot, "local-mirror", "files", "example.invalid", "big-win-ribbon.png");
  ...
  const repoMirrorAssetPath = path.relative(workspaceRoot, mirrorAssetPath).replace(/\\/g, "/");
  ...
  const originalMirrorAssetRaw = await readOptionalBuffer(mirrorAssetPath);
  ...
  const taskId = "task.runtime.mirror-reopen.project_002.smoke";
  const pageName = "big_win";

  let run: SmokeRunResult | null = null;
  let payload: MirrorReopenPayload | null = null;

  try {
    await fs.mkdir(path.dirname(sourceAssetPath), { recursive: true });
    await fs.mkdir(path.dirname(mirrorManifestPath), { recursive: true });
    await fs.mkdir(path.dirname(mirrorAssetPath), { recursive: true });

    const generatedAt = new Date().toISOString();
    await fs.writeFile(sourceAssetPath, Buffer.from("project_002_runtime_mirror_reopen_source_placeholder", "utf8"));
    await fs.writeFile(mirrorAssetPath, Buffer.from("project_002_runtime_mirror_placeholder", "utf8"));

    await fs.writeFile(
      sourceBundlePath,
      `${JSON.stringify({
        generatedAt,
        nextTextureFitApplyStep: "Open Runtime and reuse the grounded local-mirror runtime workbench entry for the selected project.",
        pages: [
          {
            pageName,
            pageState: "ready",
            selectedMode: "page-source",
            selectedReason: "Smoke-seeded selected-project runtime reopen from grounded local-mirror evidence.",
            ...
            nextFitApplyStep: "Open Runtime and keep the local-mirror-backed runtime workbench entry active while launch stays blocked."
          }
        ]
      }, null, 2)}\n`,
      "utf8"
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        ...
        readyTaskCount: 1,
        blockedTaskCount: 0,
        nextOperatorAction: "Open Runtime from the modification board and continue from the grounded local-mirror workbench entry.",
        tasks: [
          {
            taskId,
            queueId: "smoke.queue.project_002.runtime_mirror_reopen",
            scenarioId: "smoke.scenario.project_002.runtime_mirror_reopen",
            displayName: "Project 002 Runtime Mirror Reopen Smoke",
            ...
            rationale: "Smoke-seeded modification task proving selected-project runtime reopen can stay on grounded local-mirror runtime evidence while live launch stays blocked.",
            nextAction: "Open Runtime and verify the local-mirror-backed workbench entry stays project-aware.",
            canOpenCompose: true,
            canOpenRuntime: true
          }
        ]
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        ...
        resourceVersion: "project-002-mirror-reopen-smoke",
        notes: [
          "Smoke manifest for selected-project local-mirror runtime reopen verification."
        ],
        entries: [
          {
            sourceUrl: runtimeSourceUrl,
            kind: "static-image",
            repoRelativePath: repoMirrorAssetPath,
            fileType: "png"
          }
        ]
      }, null, 2)}\n`,
    ...
    run = await runElectronSelectedProjectMirrorReopenSmoke(workspaceRoot);
    ...
    assert(
      mainReadySeen,
      `Main-process selected-project mirror reopen smoke marker was not emitted. Exit=${run.exitCode} Signal=${run.signal ?? "none"}\n${output}`
    );
    ...
    assert(
      payloadLine,
      `Renderer selected-project mirror reopen smoke result marker was not emitted. Exit=${run.exitCode} Signal=${run.signal ?? "none"}\n${output}`
    );

    payload = JSON.parse(payloadLine.slice(markerPrefix.length)) as MirrorReopenPayload;
    writeArtifact(payload);

    assert.equal(payload.status, "pass", `Selected-project mirror reopen smoke payload reported failure: ${payload.error ?? "<no error>"}\n${output}`);
    assert.equal(payload.projectId, projectId, `Selected-project mirror reopen smoke did not load ${projectId}.`);
    assert.equal(payload.runtimeLaunchBlocked, true, "Selected-project mirror reopen smoke should keep launch blocked for a partial mirror.");
    assert.equal(payload.runtimeLaunchEntryUrl, null, "Selected-project mirror reopen smoke should not expose an embedded launch URL without a full mirror.");
    assert.equal(payload.pageRuntimeProofLoaded, false, "Selected-project mirror reopen smoke should not rely on a persisted page proof.");
    assert.equal(payload.runtimeWorkbenchHasPageProofEntry, false, "Selected-project mirror reopen smoke should not surface a page-proof runtime entry.");
    assert.equal(payload.taskRuntimeEntryKind, "local-mirror", "Selected-project mirror reopen smoke should report the exact local-mirror runtime trace.");
    assert.equal(payload.taskRuntimeEntrySourceUrl, runtimeSourceUrl, "Selected-project mirror reopen smoke used the wrong runtime source.");
    assert.equal(payload.runtimeWorkbenchEntryKind, "local-mirror-manifest", "Selected-project mirror reopen smoke should reopen from a grounded local-mirror manifest entry.");
    assert.equal(payload.runtimeWorkbenchEntryRequestSource, "local-mirror-manifest", "Selected-project mirror reopen smoke should preserve the local-mirror request source label.");
    assert.equal(payload.runtimeModeSelected, true, "Selected-project mirror reopen smoke did not keep the runtime workbench active.");
    assert.equal(payload.taskRuntimeOpenUsesPersistedPageProof, false, "Selected-project mirror reopen smoke should not report page-proof reopen.");
    assert.equal(payload.taskRuntimeOpenUsesRequestBackedWorkbenchEntry, false, "Selected-project mirror reopen smoke should not report request-backed reopen.");
    assert.equal(payload.taskRuntimeOpenUsesLocalMirrorWorkbenchEntry, true, "Selected-project mirror reopen smoke did not reopen from the local-mirror workbench entry.");
    assert.equal(payload.runtimeDebugHostActionVisible, true, "Selected-project mirror reopen smoke should keep the runtime Debug Host action available for project_002 when grounded runtime evidence exists.");
    ...
    assert.equal(payload.runtimeStatusSurfaceChipLabel, "local-mirror", "Selected-project mirror reopen smoke should expose the exact local-mirror runtime status chip.");
    assert.equal(payload.runtimeStatusSurfaceCardTitle, "Local-mirror runtime workbench", "Selected-project mirror reopen smoke should expose the exact local-mirror runtime status card title.");
    ...
    console.log("PASS smoke:electron-runtime-selected-project-mirror-reopen");
    ...
    await restoreOptionalBuffer(mirrorAssetPath, originalMirrorAssetRaw);
    ...
    await cleanupEmptyDirectory(path.join(reportsRoot, "selected-project-runtime-mirror-reopen"));
    ...
    assert.equal(
      restoredStatus,
      baselineStatus,
      `Selected-project mirror reopen smoke should restore the repo status.\nBefore:\n${baselineStatus || "<clean>"}\nAfter:\n${restoredStatus || "<clean>"}`
    );
  }
}
...
main().catch((error) => {
  ...
  console.error(`FAIL smoke:electron-runtime-selected-project-mirror-reopen - ${message}`);
  process.exitCode = 1;
});
