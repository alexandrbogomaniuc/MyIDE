...
interface LiveCreateDragPayload {
  status?: string;
  error?: string;
  startedAt?: string;
  projectId?: string;
...
function getArtifactPath(): string {
  return process.env.MYIDE_LIVE_CREATE_DRAG_ARTIFACT_PATH || "/tmp/myide-electron-live-create-drag.json";
}

function getScreenshotPath(): string | null {
  const value = process.env.MYIDE_LIVE_CREATE_DRAG_SCREENSHOT_PATH ?? "";
...
function runElectronLiveCreateDragSmoke(workspaceRoot: string): Promise<SmokeRunResult> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(electronBinary, ["."], {
      ...
      env: {
        ...process.env,
        MYIDE_LIVE_CREATE_DRAG_SMOKE: "1",
        MYIDE_LIVE_CREATE_DRAG_TIMEOUT_MS: process.env.MYIDE_LIVE_CREATE_DRAG_TIMEOUT_MS || "45000",
        ...(screenshotPath ? {
          MYIDE_LIVE_CREATE_DRAG_SHOW: "1",
          MYIDE_LIVE_CREATE_DRAG_KEEP_OPEN: "1"
        } : {})
      },
      stdio: ["ignore", "pipe", "pipe"]
    });
    ...
    const markerPrefix = "MYIDE_LIVE_CREATE_DRAG_RESULT:";
    ...
    const handleChunk = (chunk: unknown) => {
      ...
      while (lineBuffer.includes("\n")) {
        ...
        let payload: LiveCreateDragPayload | null = null;
        try {
          payload = JSON.parse(line.slice(markerPrefix.length)) as LiveCreateDragPayload;
        } catch (error) {
...
async function main(): Promise<void> {
  ...
  const backupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "myide-live-create-drag-backup-"));
  ...
  try {
    result = await runElectronLiveCreateDragSmoke(workspaceRoot);
    ...
    const mainReadySeen = lines.some((line) => line.includes("MYIDE_LIVE_CREATE_DRAG_MAIN_READY"));
    assert(
      mainReadySeen,
      `Main-process live create-drag marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );
    ...
    assert(
      payloadLine,
      `Renderer live create-drag result marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );

    payload = JSON.parse(payloadLine.slice(markerPrefix.length)) as LiveCreateDragPayload;
    assert.equal(payload.status, "pass", `Live create-drag smoke reported failure: ${payload.error ?? "unknown error"}`);
    ...
    assert.equal(payload.dragStarted, true, "Renderer did not start the live create-drag canvas interaction.");
    assert.equal(payload.dragMoved, true, "Renderer did not move the new object through the live pointer path.");
    assert.equal(payload.dragCompleted, true, "Renderer did not complete the live create-drag interaction.");
    assert.equal(payload.saveSucceeded, true, "Renderer did not complete save after the live create-drag.");
    assert.equal(payload.reloadSucceeded, true, "Renderer did not complete reload after the live create-drag.");
    assert.equal(payload.internalPersistVerified, true, "Renderer did not confirm the reloaded internal create-drag position.");
    assert.equal(payload.replaySyncVerified, true, "Renderer did not confirm replay-facing sync after create-drag.");

    const objectId = payload.objectId;
    assert(objectId, "Renderer did not report the created object id.");
    const expectedX = Number(payload.draggedX);
    const expectedY = Number(payload.draggedY);
    assert(Number.isFinite(expectedX) && Number.isFinite(expectedY), "Dragged renderer coordinates must be numeric.");
    ...
    onDiskInternalPosition = getInternalObjectPosition(internalDocument, objectId);
    onDiskReplayPosition = getReplayNodePosition(replayProject, objectId);
    assert.equal(onDiskInternalPosition.x, expectedX, "Editable objects.json did not persist the created object's dragged x coordinate.");
    assert.equal(onDiskInternalPosition.y, expectedY, "Editable objects.json did not persist the created object's dragged y coordinate.");
    assert.equal(onDiskReplayPosition.x, expectedX, "Replay-facing project.json did not persist the created object's dragged x coordinate.");
    assert.equal(onDiskReplayPosition.y, expectedY, "Replay-facing project.json did not persist the created object's dragged y coordinate.");
    assert.equal(
      result.exitCode,
      0,
      `Electron live create-drag smoke exited with code ${result.exitCode} (signal ${result.signal ?? "none"}).\n${outputSummary}`
    );
  } catch (error) {
    writeArtifact({
      status: "fail",
  ...
  assert.equal(
    restoredStatus,
    baselineStatus,
    `Repository status diverged after restoring the live create-drag smoke.\nBefore:\n${baselineStatus || "<clean>"}\nAfter:\n${restoredStatus || "<clean>"}`
  );
  ...
  console.log("PASS smoke:electron-live-create-drag");
  console.log(`Created and dragged object: ${payload?.objectId} (${payload?.createdX}, ${payload?.createdY}) -> (${payload?.draggedX}, ${payload?.draggedY})`);
...
main().catch((error: unknown) => {
  ...
  console.error(`FAIL smoke:electron-live-create-drag - ${message}`);
  process.exitCode = 1;
});
