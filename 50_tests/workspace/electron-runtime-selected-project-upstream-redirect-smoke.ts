...
function getArtifactPath(): string {
  return process.env.MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_ARTIFACT_PATH || "/tmp/myide-electron-runtime-selected-project-upstream-redirect.json";
}
...
function runElectronSelectedProjectRedirectSmoke(
...
async function main(): Promise<void> {
  ...
  const runtimeSourceUrl = "https://cdn.bgaming-network.com/runtime/img/big-win-ribbon.png";
  ...
  const loaderFilePath = path.join(workspaceRoot, "40_projects", projectId, "runtime", "local-mirror", "files", "cdn.bgaming-network.com", "loader.js");
  const bundleFilePath = path.join(workspaceRoot, "40_projects", projectId, "runtime", "local-mirror", "files", "cdn.bgaming-network.com", "bundle.js");
  const imageFilePath = path.join(workspaceRoot, "40_projects", projectId, "runtime", "local-mirror", "files", "cdn.bgaming-network.com", "big-win-ribbon.png");
  ...
  try {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        ...
        publicEntryUrl: "https://cdn.bgaming-network.com/runtime/launch",
        resourceVersion: "project-002-upstream-redirect-smoke",
        notes: [
          "Smoke manifest for selected-project upstream redirect verification."
        ],
        entries: [
          {
            sourceUrl: "https://cdn.bgaming-network.com/runtime/loader.js",
            kind: "runtime-loader",
            repoRelativePath: "40_projects/project_002/runtime/local-mirror/files/cdn.bgaming-network.com/loader.js",
            fileType: "js"
          },
          {
            sourceUrl: "https://cdn.bgaming-network.com/runtime/bundle.js",
            kind: "runtime-bundle",
            repoRelativePath: "40_projects/project_002/runtime/local-mirror/files/cdn.bgaming-network.com/bundle.js",
            fileType: "js"
          },
          {
            sourceUrl: runtimeSourceUrl,
            kind: "static-image",
            repoRelativePath: "40_projects/project_002/runtime/local-mirror/files/cdn.bgaming-network.com/big-win-ribbon.png",
            fileType: "png"
          }
        ]
      }, null, 2)}\n`,
        ...
        entries: [
          {
            ...
            note: "Smoke override for selected-project upstream redirect verification."
          }
        ]
      }, null, 2)}\n`,
      "utf8"
    ...
    result = await runElectronSelectedProjectRedirectSmoke(workspaceRoot, {
      ...process.env,
      MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_SMOKE: "1",
      MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_PROJECT_ID: projectId,
      MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_SOURCE_URL: runtimeSourceUrl,
      MYIDE_RUNTIME_LOCAL_MIRROR_PORT: process.env.MYIDE_RUNTIME_LOCAL_MIRROR_PORT || "39018"
    });
    ...
    const lines = result.output
      ...
      .map((line) => line.trim())
      .filter((line) => line.length > 0);
    const mainReadySeen = lines.some((line) => line.includes("MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_MAIN_READY"));
    assert(
      mainReadySeen,
      `Main-process selected-project upstream redirect smoke marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );

    const markerPrefix = "MYIDE_RUNTIME_SELECTED_PROJECT_REDIRECT_RESULT:";
    const payloadLine = lines.find((line) => line.startsWith(markerPrefix));
    assert(
      payloadLine,
      `Selected-project upstream redirect result marker was not emitted. Exit=${result.exitCode} Signal=${result.signal ?? "none"}\n${outputSummary}`
    );

    payload = JSON.parse(payloadLine.slice(markerPrefix.length)) as Record<string, unknown>;
    assert.equal(payload.status, "pass", `Selected-project upstream redirect smoke reported failure: ${String(payload.error ?? "unknown error")}`);
    assert.equal(payload.projectId, projectId, "Selected-project upstream redirect smoke reported the wrong project id.");
    assert.match(String(payload.redirectUrl ?? ""), /\/runtime\/project_002\/mirror\?source=/, "Upstream redirect did not resolve to the selected-project mirror proxy.");
    assert.equal(payload.fetchStatus, 200, "Selected-project upstream fetch did not return 200.");
    assert.match(String(payload.fetchFinalUrl ?? ""), /\/runtime\/project_002\/mirror\?source=/, "The final upstream fetch URL did not stay on the selected-project mirror proxy.");
    assert.equal(payload.matchedProjectRequestSource, "project-local-override", "Selected-project upstream redirect should honor the project-local override.");
    assert.equal(payload.project001ContainsSeedSource, false, "Selected-project upstream redirect polluted project_001 resource tracking.");
    assert.match(String(payload.fetchBodySnippet ?? ""), /project_002_override_placeholder/, "Selected-project upstream redirect did not serve the override content.");
    assert.equal(result.exitCode, 0, `Electron selected-project upstream redirect smoke exited with code ${result.exitCode} (signal ${result.signal ?? "none"}).\n${outputSummary}`);
  } catch (error) {
    writeArtifact({
      status: "fail",
    ...
    await cleanupEmptyDirectory(path.join(workspaceRoot, "40_projects", projectId, "runtime", "local-mirror", "files", "cdn.bgaming-network.com"));
    ...
    assert.equal(restoredStatus, baselineStatus, "Selected-project upstream redirect smoke did not restore the repo status.");
  }
  ...
  console.log("PASS smoke:electron-runtime-selected-project-upstream-redirect");
...
main().catch((error: unknown) => {
  ...
  console.error(`FAIL smoke:electron-runtime-selected-project-upstream-redirect - ${message}`);
  process.exitCode = 1;
});
